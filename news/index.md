...
- New function
  [`write_mats_to_excel()`](https://matthewheun.github.io/matsindf/reference/write_mats_to_excel.md)
  writes matrices in a column of a `matsindf` data frame to an Excel
  file, one matrix per worksheet.
- Changed from using
  [`dplyr::group_by_at()`](https://dplyr.tidyverse.org/reference/group_by_all.html)
  to using `dplyr::group_by(across())` for speed gain in
  [`rowcolval_to_mat()`](https://matthewheun.github.io/matsindf/reference/rowcolval_to_mat.md).
...
- [`expand_to_tidy()`](https://matthewheun.github.io/matsindf/reference/expand_to_tidy.md)
  now handles `NULL` in lists of matrices and `NULL` in a column of
  matrices in a data frame.
- [`mat_to_rowcolval()`](https://matthewheun.github.io/matsindf/reference/mat_to_rowcolval.md)
  now returns `NULL` when `NULL` is supplied.
...
- Fixed a bug wherein passing `NULL` in all named arguments to
...
- Throughout the test suite, now have better matrix equality tests that
  are agnostic to matrix class.
- Improved speed of
...
- Fixed a bug in
  [`matsindf_apply()`](https://matthewheun.github.io/matsindf/reference/matsindf_apply.md)
  where a a data frame column with some named elements and some unnamed
  elements led to the unnamed elements yielding `0` or `NULL` results
  after `FUN` was applied. The fix was to
  [`unname()`](https://rdrr.io/r/base/unname.html) everything before
  calling
  [`purrr::transpose()`](https://purrr.tidyverse.org/reference/transpose.html).
- Added a line break and separator in
  [`df_to_msg()`](https://matthewheun.github.io/matsindf/reference/df_to_msg.md)
...
- The rewrite of
  [`matsindf_apply()`](https://matthewheun.github.io/matsindf/reference/matsindf_apply.md)
  enables better error and warning messages in many places.
- Rewrote all of
  [`matsindf_apply()`](https://matthewheun.github.io/matsindf/reference/matsindf_apply.md).
  It was a complete mess! Hopefully, it is now easier to debug and
  maintain.
  - [`matsindf_apply()`](https://matthewheun.github.io/matsindf/reference/matsindf_apply.md)
    now correctly returns a list when lists are provided in the `...`
    argument. Previously, it incorrectly returned a data frame.
  - [`matsindf_apply()`](https://matthewheun.github.io/matsindf/reference/matsindf_apply.md)
    now correctly disallows any unused arguments supplied in its `...`
    argument. Previously, some unused argument situations were not
    flagged as errors, despite documentation saying they would be.
  - [`matsindf_apply()`](https://matthewheun.github.io/matsindf/reference/matsindf_apply.md)
    now gives warning messages when an argument to `FUN` cannot be found
    in any of `...`, `.dat`, or defaults to `FUN`. This condition may be
    OK, and the caller can suppress the warning by passing
    `.warn_missing_FUN_args = FALSE` to
    [`matsindf_apply()`](https://matthewheun.github.io/matsindf/reference/matsindf_apply.md).
  - [`matsindf_apply()`](https://matthewheun.github.io/matsindf/reference/matsindf_apply.md)
    now gives more descriptive error messages.
  - [`matsindf_apply()`](https://matthewheun.github.io/matsindf/reference/matsindf_apply.md)
    now allows more options for incoming data.
  - [`matsindf_apply()`](https://matthewheun.github.io/matsindf/reference/matsindf_apply.md)
    now tries really hard to deal with empty inputs, falling back to
    returning the original input unmodified (except possibly wrapped in
    a [`list()`](https://rdrr.io/r/base/list.html)) when the input has
    zero rows (in the case of a data frame) or zero length (in the case
    of lists).
...
- All functions now work with both `matrix` and `Matrix` objects,
  thereby enabling use of sparse matrices. Use
  [`matsbyname::Matrix()`](https://matthewheun.github.io/matsbyname/reference/Matrix.html)
  to create sparse matrices as appropriate.
- [`mat_to_rowcolval()`](https://matthewheun.github.io/matsindf/reference/mat_to_rowcolval.md)
  now accepts `Matrix` objects in the `.matrix` argument.
- [`rowcolval_to_mat()`](https://matthewheun.github.io/matsindf/reference/rowcolval_to_mat.md)
  gains a `matrix.class` argument that specifies whether a `matrix` or a
...
- Move tests associated with
  [`verify_cols_missing()`](https://matthewheun.github.io/matsindf/reference/verify_cols_missing.md)
  from the `Recca` package to `matsindf`. Those tests didn’t belong in
  `Recca`.
- Updated to the latest version of the github actions continuous
  integration workflow.
- Better docs for
  [`expand_to_tidy()`](https://matthewheun.github.io/matsindf/reference/expand_to_tidy.md).
- New function
  [`matrix_cols()`](https://matthewheun.github.io/matsindf/reference/matrix_cols.md)
  tells which columns contain matrices, either in the
  [`any()`](https://rdrr.io/r/base/any.html) sense or in the
  [`all()`](https://rdrr.io/r/base/all.html) sense.
- Improved
  [`matsindf_apply()`](https://matthewheun.github.io/matsindf/reference/matsindf_apply.md)
  primer vignette discusses how to handle heterogeneous arguments types
  to `FUN`.
- New tests to verify
  [`matsindf_apply()`](https://matthewheun.github.io/matsindf/reference/matsindf_apply.md)
...
- Fixed a very subtle bug in
  [`matsindf_apply()`](https://matthewheun.github.io/matsindf/reference/matsindf_apply.md)
  where the list returned from
  [`Map()`](https://rdrr.io/r/base/funprog.html) had names on each
  individual item. The names on individual items caused problems  
  only when later calling a binary (or n-ary) `matsbyname` function with
  both a named and an unnamed list, because those functions expect the
  same name structure. The solution was to remove the names from each
  individual item after calling
  [`Map()`](https://rdrr.io/r/base/funprog.html) and before calling
  [`purrr::transpose()`](https://purrr.tidyverse.org/reference/transpose.html).
- No new tests.
...
- New vignette from Alexander Davis shows how to use `matsindf` for
  principal components analysis (PCA).
- Move to `testthat` v3.
- Adapt to refactoring `RCLabels` package out of `matsbyname`.
- `rowtypes` and `coltypes` arguments now have special defaults to
  enable calling
  [`collapse_to_matrices()`](https://matthewheun.github.io/matsindf/reference/collapse_to_matrices.md)
  without specifying those arguments.
- Added more tests for
  [`collapse_to_matrices()`](https://matthewheun.github.io/matsindf/reference/collapse_to_matrices.md)
  to check whether `NULL` values for `rowtype` and `coltype` arguments
...
- New function
  ...
  [`df_to_msg()`](https://matthewheun.github.io/matsindf/reference/df_to_msg.md)
  was moved from the `SEAPSUTWorkflow` package, because it is generally
  useful.
- New tests for new features.
...
- `matsindf.Rmd` vignette now uses the `keep` argument on
  [`matsbyname::hatize_byname()`](https://matthewheun.github.io/matsbyname/reference/hatize_byname.html).
- Fixed a bug in the `matsindf.Rmd` vignette. Needed to supply the
  `keep` argument on
  [`hatize_byname()`](https://matthewheun.github.io/matsbyname/reference/hatize_byname.html).
- Added test for name collision
  [`matsindf_apply()`](https://matthewheun.github.io/matsindf/reference/matsindf_apply.md).
- New tests for new features.
...
- Move away from Travis to GitHub Actions for continuous integration
  testing.
- Added new tests for
  [`matsindf_apply()`](https://matthewheun.github.io/matsindf/reference/matsindf_apply.md)
...
- Added a new test confirming that passing a list with strings works
  with a `NULL` `.DF` in
  [`matsindf_apply()`](https://matthewheun.github.io/matsindf/reference/matsindf_apply.md).
- New tests for new features.
...
- To comply with a breaking change in `dplyr`, all calls to
  [`dplyr::group_by()`](https://dplyr.tidyverse.org/reference/group_by.html)
  now use `.add` argument instead of `add` argument.

## matsindf 0.3.5 (2020-04-13)

- Now using GitHub actions for continuous integration and building the
  pkgdown site.
...
- Maintenance release to prepare for `dplyr` 1.0.0. Several tests in
  `matsindf` assumed that some `dplyr` functions returned `data.frame`s.
  Now that many `dplyr` functions return `tibble`s, the `matsindf` tests
  needed to be rewritten. All tests pass vs. `dplyr` 0.8.5. One warning
  remains when testing `matsindf` with `dplyr` 1.0.0, namely that the
  `add` argument of `group_buy()` is deprecated. When `dplyr` 1.0.0 is
  released to CRAN, I will convert `add` to `.add`, per the `dplyr`
...
- More maintenance to prepare for R4.0.0. Prior to R4.0.0,
  [`data.frame()`](https://rdrr.io/r/base/data.frame.html) has
  `stringsAsFactors = TRUE` by default. In R4.0.0,
  `stringsAsFactors = FALSE` will be the default. In one test (“small
  example works as expected”), I was relying on the current behavior
  (`stringsAsFactors = TRUE`). That reliance has been removed so that
  this test will also pass under R4.0.0.

## matsindf 0.3.2 (2019-12-05)

CRAN release: 2019-12-06

- Maintenance release to prepare for R4.0.0. `matrix` objects now
...
- Refactored
  [`group_by_everything_except()`](https://matthewheun.github.io/matsindf/reference/group_by_everything_except.md)
  to use a new helper function
  [`everything_except()`](https://matthewheun.github.io/matsindf/reference/everything_except.md)
- New function
  [`group_by_everything_except()`](https://matthewheun.github.io/matsindf/reference/group_by_everything_except.md)
- Added CRAN badge.
...
- Address questions from CRAN: - Title case: Matrices in Data Frames -
...
- Breaking change: Default values of `rowtypes` and `coltypes` arguments
  to `expand_to_tidy` and `collapse_to_matrices` functions are no longer
  `NULL`. Rather, they are “`rowtypes`” and “`coltypes`”.

## matsindf 0.2.9 (2018-11-06)

- Extended the `matsindf_apply` primer to include sections on using
  `matsindf_apply` with a data frame and programming.

## matsindf 0.2.8 (2018-11-01)

- Added new vignette to documentation website, which required moving
  from .Rnw to .Rmd file.
...
- New function `index_column` that ratios both numbers and matrices
  relative to an initial time.
...
- `matsindf_apply` now obtains named arguments from `.DF` when a name in
  `.DF` matches a name of an argument to `FUN`. This behavior is
  overridden by supplying a string argument in `...` of
  `matsindf_apply`.

## matsindf 0.2.3 (2018-05-22)

- `matsindf_apply` now accepts a list as input when arguments are all
  strings.
