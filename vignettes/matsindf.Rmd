...
Examples of systems containing flows and nodes include
...
whose entries are not numbers but entire matrices)
is the reason <tt>matsindf</tt> exists.
...
Each flow is its own observation in the <tt>UKEnergy2000</tt> data frame,
...
|--------------------------------------------:|:--------------------------------
| <tt>matnames</tt>                           | Column of matrix names          
| <tt>rownames</tt>                           | Column of matrix row names      
| <tt>colnames</tt>                           | Column of matrix column name    
| <tt>rowtypes</tt>                           | Column of matrix row types      
| <tt>coltypes</tt>                           | Column of matrix column types   
| <tt>values</tt>                             | Column of matrix entries        
...
with the <tt>collapse_to_matrices</tt> function.
...
For ECC analyses, energy balance is verified by 
...
Seeing that energy is balanced across all ECCs, 
we proceed to other analyses.


### Efficiencies

To illustrate the power of <tt>byname</tt> in the context of <tt>matsindf</tt>,
consider the calulation of the efficiency of every industry in the ECC as a column vector.
(See Equation 11 of Heun, Owen, and Brockway [-@heun2017].)
...
Note that only a few lines of code are required to perform the same matrix
operations on every combintaion of <tt>Country</tt> and <tt>Year</tt>.
In fact, the same code works to calculate the efficiency of 
...
can be accomplished by *expand*ing the matrices back out to a tidy data frame.
...
| <tt>drop</tt>                               | The value to be dropped from output
...
<tt>etas_df</tt> is a data frame of efficiencies, 
one for each Country, Year, and Industry, 
in a format that is amenable to plotting
with tools such as [ggplot](http://ggplot2.tidyverse.org).


### Report

The following code creates a bar graph of results for the UK in 2000:
...
* Clean data into tidy data frame with columns for matrix name, 
...
* Perform other manipulations of the data.
