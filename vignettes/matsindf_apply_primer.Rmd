...
individual numbers.
This vignette demonstrates `matsindf\_apply`,
...
is a function to be applied across data, `FUN`.
...
If `example\_fun` tolerated a missing argument,
...
in the first argument to `matsindf_apply` (`.dat`).
When a value is assigned to `.dat`,
the return value from `matsindf_apply`
contains the arguments supplied to `FUN`.
