...
Passing an additional argument (\texttt{z~=~2}) causes the familiar \texttt{unused argument}
error.
...
Failing to pass a needed argument (\texttt{b = 1}) causes the familiar \texttt{missing argument} error.
