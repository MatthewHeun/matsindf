...
causes an error that indicates `b` is missing.
...
Return values indicate the actual values of arguments
that were passed to `FUN`. 
Thus, the return value has `a = 42`, 
because the value of `z` (`42`) has replaced 
the original value of `a` (`2`), 
due to `a = "z"` in `...`.
...
`example_fun_with_string()` accepts a string and a numeric.
...
the output of `matsindf_apply()` is a tibble.
